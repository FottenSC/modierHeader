...
describe('compileStateToDnr', () => {
  ...
  it('counts enabled header overwrite rules for the action badge', () => {
    const state = createDefaultState();
    state.profiles[0].rules.push(
      createEmptyRule({ headerName: 'x-one' }),
      createEmptyRule({ kind: 'responseHeader', headerName: 'x-two' }),
      createEmptyRule({ operation: 'append', headerName: 'accept-language' }),
      createEmptyRule({ operation: 'remove', headerName: 'x-remove', headerValue: '' }),
      createEmptyRule({ kind: 'redirect', redirectRegexSubstitution: 'https://new.example/\\1' }),
      createEmptyRule({ enabled: false, headerName: 'x-disabled' }),
    );

    expect(countOverwrittenHeaders(state)).toBe(2);

    state.enabled = false;
    expect(countOverwrittenHeaders(state)).toBe(0);
  });
});
